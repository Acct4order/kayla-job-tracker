...
const generatePDF = async (resumeText, job, setDl) => {
  ...
  try {
    ...
    for(const raw of lines){
      ...
      else if(t==='section'){chk(16);y+=4;doc.setFillColor(...NAVY);doc.rect(ml,y-3.5,3,5,'F');doc.setFont('helvetica','bold');doc.setFontSize(9.5);doc.setTextColor(...NAVY);doc.text(line.toUpperCase(),ml+6,y);y+=3;doc.setDrawColor(...LGRAY);doc.setLineWidth(0.3);doc.line(ml+6,y,W-mr,y);y+=5;doc.setTextColor(...DARK);}
...
const runATSCheck = text => {
  const lines=text.split('\n').map(l=>l.trim()).filter(Boolean),up=text.toUpperCase(),wc=text.split(/\s+/).filter(Boolean).length;
