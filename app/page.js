...
const getWorkMode = j => {
  ...
  if (d.indexOf('hybrid') >= 0) return 'Hybrid';
  if (d.indexOf('work from home') >= 0) return 'Remote';
  return 'On-site';
};
const fmtSalary = j => {
  if (!j.job_min_salary && !j.job_max_salary) return 'Not listed';
  const cur = j.job_salary_currency || 'CAD', per = j.job_salary_period === 'YEAR' ? '/yr' : j.job_salary_period === 'HOUR' ? '/hr' : '';
  const f = n => '$' + Number(n).toLocaleString();
  if (j.job_min_salary && j.job_max_salary) return cur + ' ' + f(j.job_min_salary) + '-' + f(j.job_max_salary) + per;
  return cur + ' ' + f(j.job_min_salary || j.job_max_salary) + per;
};
...
const mergeJobs = (existing, incoming) => {
  const idSet = new Set(existing.map(j => j.id)), hashSet = new Set(existing.map(dedupKey));
  let dupes = 0;
  const unique = incoming.filter(j => { if (idSet.has(j.id) || hashSet.has(dedupKey(j))) { dupes++; return false; } idSet.add(j.id); hashSet.add(dedupKey(j)); return true; });
  return { unique, dupes };
};
...
const runATSCheck = text => {
  ...
  const checks = [
    ...
    c('Single-column layout', !text.split('\n').some(l => (l.match(/\|/g) || []).length > 3), 'Single-column structure', 'Multi-column layout detected'),
